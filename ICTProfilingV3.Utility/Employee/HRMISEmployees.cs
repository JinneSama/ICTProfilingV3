...
namespace ICTProfilingV3.Utility.Employee
{
    public static class HRMISEmployees
    {
        ...
        private static HRMISService _service;
        public static async Task InitContext(HRMISService service)
        {
            _service = service;
        ...
        {
            var division = await _service.GetDivision();
            var offices = await _service.GetOffice();
